function createBubbleChart(error, countries, continentNames) {
  ...
  function updateCircles() {
    circles.attr("fill", function(d) {
      return flagFill()
        ? "url(#" + d.CountryCode + ")"
        : continentColorScale(d.ContinentCode);
    });
  }
  ...
  function createForces() {
    ...
    function createPopulationForces() {
      var continentNamesDomain = continents
        .values()
        .map(function(continentCode) {
      ...
      return {
        x: d3
          .forceX(function(d) {
            return (
              populationScaleX(continentNames[d.ContinentCode]) +
  ...
  function createForceSimulation() {
    console.log("createForceSimulation");
    forceSimulation = d3
      .forceSimulation()
  ...
  function forceCollide(d) {
    return countryCenterGrouping() || populationGrouping()
      ? 0
      : circleRadiusScale(d.Population) + 1;
  }
